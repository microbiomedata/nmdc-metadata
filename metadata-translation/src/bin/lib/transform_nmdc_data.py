...
def make_dict_from_nmdc_obj(nmdc_obj):
        def make_dict(obj):
            ...
            obj_dict = {}
            temp_dict = jsonasobj.as_dict(nmdc_obj)
            for k, v in temp_dict.items():
                if type([]) == type(v) and len(v) > 0: # check for empty list
                    obj_dict[k] = v
                elif pds.notnull(v) and len(v) > 0: # check for null values and empty dict
                    obj_dict[k] = v

            return obj_dict

        if type([]) == type(nmdc_obj):
            nmdc_dict = [jsonasobj.as_json_obj(o) for o in nmdc_obj]
...
def make_object_from_dict(nmdc_record:namedtuple, object_dict:dict, uriorcurie=''):
    ...
    if 'init' in object_dict.keys():
        ...
        class_type = nmdc.AttributeValue; obj = class_type() # create object from type
