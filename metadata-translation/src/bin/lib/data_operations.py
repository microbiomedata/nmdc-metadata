...
def make_nmdc_dict_list (dictionary,
    ...
    def make_constructor_args (constructor_map, record):
        ...
        for key, field in constructor_map.items():
            ## check if constructor param is a dict; dicts are handled differently
            if type({}) != type(field):
                ## if the fiels is a tuple, index 0 is param dict, index 1 is the class
                ## e.g., lat_lon': ('lat_lon', nmdc.GeolocationValue)
                if type(()) == type(field):
                    params = { key: record[value] for key, value in  field[0].items()}
                    constructor_dict[key] = field[1](**params)
                else:
                    # print(field)
