...
def make_dataframe_from_spec_file (data_spec_file):
    
    def make_df_from_file(data_source):
        ...
        if 'file_archive_name' in data_source.keys(): 
            ...
            df = make_dataframe(fname, file_archive_name=farchive, file_type=file_type)
        else:
            df = make_dataframe(fname, file_type=file_type)
    ...
    def make_df (source, source_type='file_name'):
        ...
        if 'file_name' in data_source.keys():
            df = make_df_from_file(data_source)
...
def merge_dataframes (dataframes, data_source_names=[]):
    ...
    merged_df = pds.DataFrame(columns=['data_source', 'variable', 'value'])
    ...
    for idx, df in enumerate(dataframes):
        ...
        eavdf = data.melt()
        eavdf['data_source'] = data_source_name
