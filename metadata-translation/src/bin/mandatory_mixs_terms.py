...
@click.option("--sheet-number", "-sheet", default=2,  \
              help="specifies the sheet number to process for Excel files")
@click.option("--output-format", "-of", default="tsv", \
...
def main(input, output, delimiter, sheet_number, output_format):
    ...
    if '.xls' in file_ext.strip():
        data = dop.make_dataframe(input.strip(), file_type='excel', sheet_name=sheet_number)
    else:
