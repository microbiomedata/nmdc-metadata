...
import jsonasobj
from nmdc import Biosample, GeolocationValue, ControlledTermValue, QuantityValue, Activity, OntologyClass
...
def align_biosample(file_path):
    ...
    for item in json_list:
        ...
        if 'depth' in item.keys(): item['depth'] = attribute_value_to_datatype(item['depth'], 'quantity value', 'm')
